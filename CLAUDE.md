...
- **Model selection is per-session, Provider instance is shared.** `.invox/providers.json` / `~/.invox/providers.json` defines providers and the model menu; the Provider reads `req.model` per call and falls back to its constructor default if unset. Adding a *different* baseURL/apiKey still requires a process restart — only the model id is hot-swappable. Agent tier routing lives in `providers.json.agentModels` (`PRO` / `LITE`).
